...
fields = ['участок', 'фамилия', 'шифр_детали', 'наименование_детали', 'операция', 'норма_времени_на_единицу', 'номер_ТБ_инструкции', 'статус']
tables = ['участки', 'работники', 'детали', 'детали', 'операции', 'операции', 'инструкции', 'статус']
columns = ['участок', 'фамилия', 'шифр_детали', 'наименование_детали', 'операция', 'норма_времени_на_единицу', 'номер_ТБ_инструкции', 'статус']

for field, table, column in zip(fields, tables, columns):
...
def update_completer(field, table, column, entry):
    c.execute(f'SELECT DISTINCT {column} FROM {table}')
    data = c.fetchall()
    completer = QCompleter([str(row[0]) for row in data])
    completer.setCaseSensitivity(Qt.CaseInsensitive)
...
def save_data():
    try:
        ...
        ''')
        c.execute(f'''
            INSERT INTO инструкции (номер_ТБ_инструкции) VALUES
            ({entries["номер_ТБ_инструкции"].text()})
        ''')
        c.execute(f'''
            INSERT INTO статус (статус) VALUES
            ('{entries["статус"].text()}')
        ''')
        conn.commit()
        # Обновите данные автозаполнения
        for field, table, column in zip(fields, tables, columns):
            update_completer(field, table, column, entries[field])
